...
#     check = input("Do you want to add another input? (Y/N) ").lower()
#     if check == "n":
#         break
...
search = input("What colour do you want to search? ").lower()
count = 0

for colours in favourite_colours:
    if favourite_colours[colours] == search:
        count += 1
if count != 0:
    print(f"Enter colour ton search: {search}")
    print(f"{count} students like {search}.")
    print(f"Students who like {search}:")
    for name in favourite_colours:
        print(f"  {name}")
else:
    print(f"No one likes the colour {search}")
