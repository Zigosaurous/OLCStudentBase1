...
valid = 0
check = 0 #1 bring it offside the loop
...
for i in range(len(egg_code)):
    ...
    if egg_code[i][0] in [0, 1, 2, 3]:
        check += 1
    if egg_code[i][1:3].isalpha(): #2 added ()
        check += 1
    if egg_code[i][3:].isdigit(): #3 added ()
        check += 1
    if check == 3:
        valid = 1
if valid == 1: #4 change to 1
    ...
    for m in range(len(egg_code)):
        for n in range(len(countries)):
            if egg_code[m][:2] == countries[n]:
                countries_eggs[m] += 1 #5 added a indent
